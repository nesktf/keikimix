...
int main() {
  ...
  auto cousine = shogle::load_font_atlas<char>("./res/CousineNerdFont-Regular.ttf");
  if (!cousine) {
    ntf::logger::error("[main] Failed to load font: {}", cousine.error().what());
    return EXIT_FAILURE;
  }

  auto cirno_img = shogle::load_image<ntf::uint8>("./res/cirno_cpp.jpg");
  if (!cirno_img) {
    ntf::logger::error("[main] Failed to load cirno image: {}", cirno_img.error().what());
    return EXIT_FAILURE;
  }

  auto atlas = shogle::load_atlas("./res/2hus.json", shogle::atlas_load_flags::flip_y);
  if (!atlas) {
  ...
  auto vert_src = shogle::file_contents("./res/shaders/vert_base.vs.glsl").value();
  auto frag_col_src = shogle::file_contents("./res/shaders/frag_color.fs.glsl").value();
  auto frag_tex_src = shogle::file_contents("./res/shaders/frag_tex.fs.glsl").value();
  auto vert_atl_src = shogle::file_contents("./res/shaders/vert_atlas.vs.glsl").value();

  const auto fumo_flag = shogle::model_load_flags::triangulate;
  auto fumo = shogle::load_model<shogle::pnt_vertex>("./res/cirno_fumo/cirno_fumo.obj", fumo_flag);
  if (!fumo) {
